...
{
    public class NetworkCarComponent : NetworkEntity
    {
        ...
        public NetworkCarAttacker carAttacker;
        ...
        public bool IsInWitchTime { get; private set; }
        ...
        {
            var randomValue = Random.Range(0, 2) == 0 ? -1 : 1;
            bodyRb.mass *= 2;
        ...
        private void RotateRigidbody(Rigidbody rb, float angle, float duration)
        {
            // 将角度转换为弧度
            var angleInRadians = angle * Mathf.Deg2Rad;
            // 计算所需的角速度
            var angularVelocity = angleInRadians / duration;
            // 设置 Rigidbody 的角速度
