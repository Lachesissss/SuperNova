...
{
    public sealed class NetworkCarPlayer : NetworkCarController
    {
        private static bool isP1Exist;
        private static bool isP2Exist;
        ...
        public override void OnUpdate(float elapseSeconds, float realElapseSeconds)
        {
            ...
            if (Boost) TryBoost();
            if (Switch) TrySwitch();
            if (Skill1) ActivateSkill(0);
            if (Skill2) ActivateSkill(1);
            if (Skill3) ActivateSkill(2);
        }
        ...
        private void TrySwitch()
        {
            ...
            var randomCooling = Random.Range(m_globalConfig.carSwitchCoolingTimeMin, m_globalConfig.carSwitchCoolingTimeMax);
            var coolingInfo = new CoolingInfo();
            coolingInfo.playerName = controllerName;
        ...
        {
            if (userData is NetworkCarControllerData { userData: PlayerType type })
            {
                if (type == PlayerType.P1)
                {
                    if (isP1Exist)
                    {
                        Debug.LogError("出错了，P1已存在");
                    }
                    else
                    {
                        isP1Exist = true;
